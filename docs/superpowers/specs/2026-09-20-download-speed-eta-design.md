...
- **速度**：匹配 `at` 与 `/s` 之间的片段；将 `KiB/s`/`MiB/s`/`GiB/s` 规范为 `KB/s`/`MB/s`/`GB/s`（数值保留约一位小数，过小可保留整数）
- **ETA**：匹配 `ETA` 后的 `HH:MM:SS` 或 `MM:SS`；去掉无意义前导零小时（`00:25` → `0:25` 或 `25s` 二选一，实现定为 `0:25` 以保持时钟风格）；未知/不可用（如 `Unknown`、`--:--`）视为 `null`
